...
export const createTestDatabase = async (): Promise<void> => {
  const adminPool = new Pool({
    connectionString: process.env.DATABASE_URL?.replace('coursedb', 'postgres') || 'postgresql://courseuser:coursepass@localhost:5432/postgres'
  });

  try {
    // Close existing test pool if it exists
    if (poolInstance) {
    ...
    // Silently continue if database doesn't exist or other errors occur
    // The schema init will handle creating tables
  } finally {
    await adminPool.end();
  }
};
