...
const TEST_DATABASE_URL =
  process.env.DATABASE_URL?.replace("usersdb", "usersdb_test") ||
  "postgresql://courseuser:coursepass@localhost:5432/usersdb_test";
...
export const createTestDatabase = async (): Promise<void> => {
  const adminPool = new Pool({
    connectionString:
      process.env.DATABASE_URL?.replace("usersdb", "postgres") ||
...
export const cleanTestData = async (): Promise<void> => {
  await testPool.query("TRUNCATE subscriptions RESTART IDENTITY CASCADE");
