...
describe("GET /api/catalog", () => {
  ...
  it("should include is_liked for authenticated user", async () => {
    ...
    const likedCourse = response.body.courses.find(
      (c: any) => c.id === courses[0].id,
    );
    expect(likedCourse.is_liked).toBe(true);

    const notLikedCourse = response.body.courses.find(
      (c: any) => c.id === courses[1].id,
    );
  ...
  it("should show correct like counts", async () => {
    ...
    const course = response.body.courses.find(
      (c: any) => c.id === courses[0].id,
    );
